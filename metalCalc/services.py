...
def _calculate_weight_of_metal(volume: float, metal_type_selected: int, metal_alloy_selected: int) -> str:
    ...
    :param volume: volume of metal
    :param metal_type_selected: metal ID in the database
    :param metal_alloy_selected: metal alloy ID in the database
    :return: metal weight, based on data entered by the user
    """
    if metal_alloy_selected == 0:
        density: int = METAL_TYPES.get(pk=metal_type_selected).get('density')
    else:
        density: int = METAL_ALLOYS.get(pk=metal_alloy_selected).get('density')
    return "{0:.2f}".format(round(volume * density, 2))
...
def _get_context_data_for_calculator_fields(request: dict, context: dict) -> dict:
    """
    ...
    if shape_selected == 1:
        if width < s2 or height < (s * 2):
            ...
            volume: float = (((width * height) - 2 * ((height - 2 * s) * ((width - s2) / 2))) / 1000000) * length
    elif shape_selected == 2:
        volume: float = ((width ** 2) / 1000000) * length
    elif shape_selected == 3:
        volume: float = (3.141592653589793 * diameter ** 2 / 4 / 1000000) * length
    elif shape_selected == 4:
        volume: float = (width * height / 1000000) * s * quantity / 1000
    elif shape_selected == 5:
        volume: float = (width * length / 1000) * s / 1000
    elif shape_selected == 6:
        if diameter < s * 2:
            volume: float = 0.0
            context['error_message'] = True
        else:
            volume: float = (3.141592653589793 * (diameter ** 2 - (diameter - s * 2) ** 2) / 4 / 1000000) * length
    elif shape_selected == 7:
        if min(width, height) < s * 2:
            volume: float = 0.0
            context['error_message'] = True
        else:
            volume: float = (width * height - (width - s * 2) * (height - s * 2)) / 1000000 * length
    elif shape_selected == 8:
        if min(width, height) < s:
            volume: float = 0.0
            context['error_message'] = True
        else:
            volume: float = (((width * s) + ((height - s) * s)) / 1000000) * length
    elif shape_selected == 9:
        if width < s * 2 or height < s:
            volume: float = 0.0
            context['error_message'] = True
        else:
            volume: float = (((width * s) + ((height - s) * s * 2)) / 1000000) * length
    elif shape_selected == 10:
        volume: float = (2 * (3 ** 0.5) * ((diameter / 1000) / 2) ** 2) * length
    else:
        volume: float = 0.0

    weight: str = _calculate_weight_of_metal(volume=volume,
                                             metal_type_selected=metal_type_selected,
                                             metal_alloy_selected=metal_alloy_selected)
